...
public class SeekBarDialogPreference extends DialogPreference implements SeekBar.OnSeekBarChangeListener {
    ...
    protected void onDialogClosed(boolean positiveResult) {
        ...
        if (positiveResult && shouldPersist()) {
            ...
            notifyChanged(); // aggiorna il summary con il nuovo valore
        }
    }

    @Override
    public CharSequence getSummary() {
        int raw = getPersistedInt(mDefault);
        int displayed = (mMultiplier > 1) ? (raw / mMultiplier) : raw;
        String val = String.valueOf(displayed);
        return mSuffix != null ? val.concat(mSuffix) : val;
    }
}
